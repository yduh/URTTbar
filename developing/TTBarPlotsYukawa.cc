...
TTBarPlotsYukawa::TTBarPlotsYukawa(string prefix) : prefix_(prefix), plot1d(prefix), plot2d(prefix)
{

}

TTBarPlotsYukawa::~TTBarPlotsYukawa()
{

}

void TTBarPlotsYukawa::Init(ttbar* analysis)
{
	an = analysis;
    ...
    plot1d.AddHist("Mtt", 1000, 0, 2000, "M(t#bar{t}) [GeV]", "Events");
    plot1d.AddHist("dely", 1200, -6., 6., "#Deltay(t#bar{t})", "Events");
    plot2d.AddHist("Mtt_dely", 1000, 0, 2000, 1200, -6., 6., "M(t#bar{t}) [GeB]", "#Deltay(t#bar{t})");
    ...
    plot1d.AddHist("mu_pt", 500, 0., 500., "p_{T}(l) [GeV]", "Events");
    plot1d.AddHist("mu_eta", 480, -2.4, 2.4, "#eta(l)", "Events");
    plot1d.AddHist("el_pt", 500, 0., 500., "p_{T}(l) [GeV]", "Events");
    plot1d.AddHist("el_eta", 480, -2.4, 2.4, "#eta(l)", "Events");
    ...
    plot1d.AddHist("jet1", 160, 0., 800., "1. p_{T}(jet) [GeV]", "Events");
    plot1d.AddHist("jet2", 160, 0., 800., "2. p_{T}(jet) [GeV]", "Events");
    plot1d.AddHist("jet3", 160, 0., 800., "3. p_{T}(jet) [GeV]", "Events");
    plot1d.AddHist("jet4", 160, 0., 800., "4. p_{T}(jet) [GeV]", "Events");
}

//void TTBarPlotsYukawa::Fill(TLorentzVector* Hb, TLorentzVector* Hwa, TLorentzVector* Hwb, TLorentzVector* Lb, TLorentzVector* Ll, TLorentzVector* Ln, int lepcharge, double weight)
void TTBarPlotsYukawa::Fill(Permutation& per, double weight)
{
	...
	plot1d["Mtt"]->Fill(per.TT().M(), weight);
	plot1d["delY"]->Fill(per.T().Rapidity()-per.Tb().Rapidity(), weight);
	plot1d["Mtt_delY"]->Fill(per.TT().M(), per.T().Rapidity()-per.Tb().Rapidity(), weight);
	...
	vector<double> pts = {per.BHad()->Pt(), per.BLep()->Pt(), per.WJa()->Pt(), per.WJb()->Pt()};
	sort(pts.begin(), pts.end());
	plot1d["jet1"]->Fill(pts[3], weight);
	plot1d["jet2"]->Fill(pts[2], weight);
	plot1d["jet3"]->Fill(pts[1], weight);
	plot1d["jet4"]->Fill(pts[0], weight);
