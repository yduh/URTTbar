...
void TTBarPlots::Fill(TLorentzVector* Hb, TLorentzVector* Hwa, TLorentzVector* Hwb, TLorentzVector* Lb, TLorentzVector* Ll, TLorentzVector* Ln, int lepcharge, double test, OJet* bjet)
{
	if(bjet != 0)
	{
		if(bjet->BTagCSVv2() > 0.941)
		{
	...
	TTSemi* A = dynamic_cast<TTSemi*>(GLAN);
	if(A->SEMILEP)
	{
		TLorentzVector genthad(*(A->GenBh) + *(A->GenWPartons[0]) + *(A->GenWPartons[1]));
		plot1d["res_Mtt"]->Fill((genthad.M() -  thad.M())/genthad.M());
		plot1d["res_ptt"]->Fill((genthad.Pt() -  thad.Pt())/genthad.Pt());
		if(A->GenWPartons[0]->DeltaR(*Hwa) < A->GenWPartons[1]->DeltaR(*Hwa))
		{
			plot1d["res_wjets"]->Fill((A->GenWPartons[0]->Pt() -  Hwa->Pt())/A->GenWPartons[0]->Pt());
			plot1d["res_wjets"]->Fill((A->GenWPartons[1]->Pt() -  Hwb->Pt())/A->GenWPartons[1]->Pt());
		}
		else
		{
			plot1d["res_wjets"]->Fill((A->GenWPartons[0]->Pt() -  Hwb->Pt())/A->GenWPartons[0]->Pt());
			plot1d["res_wjets"]->Fill((A->GenWPartons[1]->Pt() -  Hwa->Pt())/A->GenWPartons[1]->Pt());
