	...
	cmumumassmax(1000.)
{
	...
	TR_ISOMu24 = 1<<6;
	TR_Mu17Mu8_L1 = 1<<7;
	TR_Mu17Mu8_L1gt10 = 1<<8;
	TR_Mu17Mu8_L2DMF = 1<<9;
	TR_Mu17Mu8_L2SMF = 1<<10;
	TR_Mu17Mu8_L3DMF = 1<<11;
	TR_Mu17Mu8_L3SMF = 1<<12;
	DZtest = 1<<13;
	DZpos = 1<<14;
	TR_ISOMu24_test = 1<<15;
	TR_Mu17Mu8_L1_test = 1<<16;
	TR_Mu17Mu8_L1gt10_test = 1<<17;
	TR_Mu17Mu8_L2DMF_test = 1<<18;
	TR_Mu17Mu8_L2SMF_test = 1<<19;
	TR_Mu17Mu8_L3DMF_test = 1<<20;
	TR_Mu17Mu8_L3SMF_test = 1<<21;
	MCTRUTH = 1<<22;

	isomuvec.push_back("HLT_IsoMu24_IterTrk02_v1:hltL3crIsoL1sMu16L1f0L2f16QL3f24QL3crIsoRhoFiltered0p15IterTrk02");
	dmfvec.push_back("HLT_Mu17_Mu8_v1:hltL3pfL1sDoubleMu103p5L1f0L2pf0L3PreFiltered8");
	smfvec.push_back("HLT_Mu17_Mu8_v1:hltL3fL1sDoubleMu103p5L1f0L2f10OneMuL3Filtered17");
...
void DYTPMu::FillTPTree()
{
	...
	for(size_t i = 0 ; i < CutTracks.size() ; i++)
	{
		...
		for(size_t j = 0 ; j < CutMuonTracks.size() ; j++)
		{
			...
			//ID	
			if(!sa.IsGoodTRK() || !sa.IsMatched()) continue;
			tpresult |= ID;
			ppx = sa.Px();
			ppy = sa.Py();
			ppz = sa.Pz();
...
Int_t DYTPMu::AnalyseEvent()
{
	...
	if(testrun != Run() && RecMus.size() == 2)
	{
		...
		PrintTriggerFilter("HLT_IsoMu24_IterTrk02_v");
		PrintTriggerFilter("Mu17_Mu8");
	}
	FillTPTree();
	DZTest();
	return(1);
